...
def delete_review(request, user_id, review_id):
    ...
    if not request.user.is_authenticated:
        # Check if this is a demo_user request (user_id is 'demo_user' string or matches demo user)
        try:
            demo_user = User.objects.get(username='demo_user')
            if str(user_id) == 'demo_user' or str(user_id) == str(demo_user.id):
...
def demo_user_watchlist(request):
    ...
    items = UserWatchlist.objects.filter(user=demo_user)
    return JsonResponse({
        'items': [{
            'id': item.id,
            'tmdbId': item.tmdb_id,
...
@csrf_exempt
@require_http_methods(["GET", "POST"])
def demo_user_favorites(request):
    demo_user = get_or_create_demo_user()
    ...
    items = UserFavorites.objects.filter(user=demo_user)
    return JsonResponse({
        'items': [{
            'id': item.id,
            'tmdbId': item.tmdb_id,
...
def demo_user_watched(request):
    ...
    if request.method == "POST":
        try:
            ...
            item, created = ViewingHistory.objects.get_or_create(
                user=demo_user,
                tmdb_id=tmdb_id,
                media_type=media_type,
                defaults={'title': title, 'poster_path': poster_path}
    ...
    items = ViewingHistory.objects.filter(user=demo_user)[:50]
    return JsonResponse({
        'items': [{
            'id': item.id,
            'tmdbId': item.tmdb_id,
...
@csrf_exempt
@require_http_methods(["DELETE"])
def demo_user_watchlist_remove(request, tmdb_id):
    demo_user = get_or_create_demo_user()
    deleted, _ = UserWatchlist.objects.filter(
        user=demo_user,
        tmdb_id=tmdb_id
...
@csrf_exempt
@require_http_methods(["DELETE"])
def demo_user_favorites_remove(request, tmdb_id):
    demo_user = get_or_create_demo_user()
    deleted, _ = UserFavorites.objects.filter(
        user=demo_user,
        tmdb_id=tmdb_id
...
@csrf_exempt
@require_http_methods(["DELETE"])
def demo_user_watched_remove(request, tmdb_id):
    demo_user = get_or_create_demo_user()
    deleted, _ = ViewingHistory.objects.filter(
        user=demo_user,
        tmdb_id=tmdb_id
...
@require_GET
def demo_user_ratings(request):
    """Get demo user ratings/reviews"""
    demo_user = get_or_create_demo_user()
    reviews = UserReview.objects.filter(user=demo_user)
    return JsonResponse({
        'ratings': [{
            'id': review.id,
            'tmdbId': review.tmdb_id,
...
@csrf_exempt
def demo_user_profile(request):
    """Get or update demo user profile"""
    demo_user = get_or_create_demo_user()
    ...
    if request.method == 'PATCH':
        try:
            data = json.loads(request.body)
            if 'firstName' in data:
                demo_user.first_name = data['firstName']
            if 'lastName' in data:
                demo_user.last_name = data['lastName']
            if 'bio' in data:
                profile.bio = data['bio']
            if 'profileImageUrl' in data:
                profile.profile_image_url = data['profileImageUrl']
    ...
    from .models import UserActivityStats, UserBadge
    
    watchlist_count = UserWatchlist.objects.filter(user=demo_user).count()
    favorites_count = UserFavorites.objects.filter(user=demo_user).count()
    reviews_count = UserReview.objects.filter(user=demo_user).count()
    watched_count = ViewingHistory.objects.filter(user=demo_user).count()
    ...
    return JsonResponse({
        'user': {
            ...
            'bio': profile.bio,
            'profileImageUrl': profile.profile_image_url,
            'createdAt': demo_user.date_joined.isoformat(),
        },
        'statistics': {
            'totalWatched': watched_count,
            'watchlistCount': watchlist_count,
...
@csrf_exempt
@require_http_methods(["GET", "POST"])
def demo_user_reviews(request):
    """Get or create reviews for demo user"""
    demo_user = get_or_create_demo_user()
    
    if request.method == "GET":
        reviews = UserReview.objects.filter(user=demo_user).order_by('-created_at')
        return JsonResponse({
            'reviews': [{
                'id': review.id,
                'tmdbId': review.tmdb_id,
...
@csrf_exempt
@require_http_methods(["DELETE"])
def demo_user_delete_review(request, review_id):
    """Delete a review for demo user"""
    demo_user = get_or_create_demo_user()
